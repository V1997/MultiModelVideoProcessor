...
class VideoToChatWorkflowTester:
    ...
    async def test_video_metadata_access(self, video_id: int) -> Dict[str, Any]:
        ...
        # Method 2: Try status endpoint as fallback
        try:
            async with self.session.get(f"{API_BASE}/video/{video_id}/status") as response:
                if response.status == 200:
                    status_data = await response.json()
                    print(f"✅ Video status from API: {status_data}")
                    return status_data
        except Exception as e:
